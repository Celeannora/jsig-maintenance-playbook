...
└── mrc-cards/                          # Consolidated umbrella for all three generated MRC card families (160 cards total)
    ...
    │                                   # (INDEX.md also cross-links the ops/ and network-infra/ tables below,
    │                                   #  making it a combined 160-card index across all three families)
...
Outputs land inside each family's own folder (e.g. `mrc-cards/master/MRC-HANDBOOK-MASTER.docx`/`.pdf`). The `.docx` is committed (the distributable deliverable); the `.pdf` companion is gitignored — a regenerable binary artifact, not source of truth. Do not hand-edit a handbook; edit the source `.md` cards (or their upstream calendar/runbook) and regenerate both the cards and the handbook, then re-commit the refreshed `.docx`.
...
- **`mrc-cards/{master,ops,network-infra}/MRC-HANDBOOK-*.docx`** — one bound, human-readable handbook per family (cover page, clickable Table of Contents, real Word tables, page-per-card layout) rendered straight from the same 160 `.md` cards above — nothing hand-typed. Committed to source control as the distributable deliverable. The `.pdf` companion (`soffice --headless --convert-to pdf` on the `.docx`) is gitignored — regenerate it locally on demand. Generated by `tools/render_mrc_handbook.py`.
