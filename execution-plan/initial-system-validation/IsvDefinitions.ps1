...
$Script:Sections = @(
    @{
        ...
        Items = @(
            ...
            @{ Id='0.J'; Req='System membership and physical network adapters: record domain/workgroup status plus adapter name, manufacturer, and type only.'; Method='Native -- PowerShell CIM'; Command='Get-CimInstance -ClassName Win32_ComputerSystem | Select-Object Domain,PartOfDomain,Workgroup | Format-Table -AutoSize | Out-String -Width 200; Get-CimInstance -ClassName Win32_NetworkAdapter | Where-Object { $_.PhysicalAdapter } | Select-Object Name,Manufacturer,AdapterType | Format-Table -AutoSize | Out-String -Width 200' }
        )
    }
    @{
        ...
        Items = @(
            ...
            @{ Id='1.D'; Req='Password-protect the BIOS administrator/setup account with the current approved password; test access without exposing the secret in the report.'; Method='Vendor BIOS provider (Dell DCPP / HP CMSL, auto-detected)'; Command='if (Get-Module -ListAvailable -Name DellBIOSProvider) { Import-Module DellBIOSProvider; Get-Item DellSmbios:\Security\IsAdminPasswordSet | Out-String } elseif (Get-Module -ListAvailable -Name HP.ClientManagement) { Import-Module HP.ClientManagement; Get-HPBIOSSetupPasswordIsSet | Out-String } else { throw "Neither DellBIOSProvider nor HP.ClientManagement module is installed on this system." }' }
            @{ Id='1.E'; Req='Verify the BIOS user/power-on password, if present, cannot modify BIOS settings; apply the approved read-only or no-access control without exposing the secret in the report.'; Method='Vendor BIOS provider (Dell DCPP / HP CMSL, auto-detected)'; Command='if (Get-Module -ListAvailable -Name DellBIOSProvider) { Import-Module DellBIOSProvider; Get-Item DellSmbios:\Security\IsSystemPasswordSet | Out-String; Get-Item DellSmbios:\Security\AdminSetupLockout | Out-String } elseif (Get-Module -ListAvailable -Name HP.ClientManagement) { Import-Module HP.ClientManagement; Get-HPBIOSPowerOnPasswordIsSet | Out-String } else { throw "Neither DellBIOSProvider nor HP.ClientManagement module is installed on this system." }' }
        )
    }
    @{
        Title = '2. Services'
...
Get-Service | Where-Object { $targets -contains $_.DisplayName } | Select-Object DisplayName,Name,Status,StartType'
    ...
    @{
        ...
        Note  = 'For a Trellix/McAfee-managed endpoint (the vendor named on the source checklist), DAT currency is read from the local agent''s registry state -- vendor-agnostic per this repo''s existing backup/AV documentation standard, this same pattern applies to any managed endpoint agent. If the registry path is absent (e.g. a different AV vendor, or Defender-only), this resolves to N/A rather than a false PASS/FAIL -- adjust the registry path for your deployed agent.'
        Items = @(
            @{
                Id  = '6'
                Req = 'Anti-virus signature file current within 30 days'
                Method = 'Native -- PowerShell reg query against the local AV engine''s registry state (Trellix/McAfee: HKLM\SOFTWARE\WOW6432Node\McAfee\AVEngine; Defender-coexistence check: Get-MpComputerStatus)'
                Command = '$dat = Get-ItemProperty -Path ''HKLM:\SOFTWARE\WOW6432Node\McAfee\AVEngine'' -Name AVDatDate -ErrorAction Stop
$dat.AVDatDate'
                Evaluate = {
                    param($Objects, $Text)
                    $raw = ($Text -replace '(?m)^.*AVDatDate.*$','').Trim()
                    if ([string]::IsNullOrWhiteSpace($raw)) { $raw = $Text.Trim() }
                    $parsed = $null
