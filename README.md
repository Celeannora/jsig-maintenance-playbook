...
```
reference/                          # Consolidated umbrella for all offline reference & research material
  JSIG/                              # Control-family reference (26 families, 274 controls)
  ...
  JSIG-source/                       # Verbatim/near-verbatim JSIG PDF extractions (offline copies)
  ...
  external-sources/                  # Offline copies of every external source cited in the research below
  ...
  research/                          # Raw research inputs/scripts behind the cadence research file below
    operational-maintenance-schedules-research.md    # Cadence research compiled from all sources above
    operational-runbook-and-artifact-standards.md    # Runbook/artifact authoring-standards research
    source_notes/, _intermediate/, _research_scripts/  # Raw fetched source notes + one-off collection scripts
  SOURCES-LEDGER.md                  # Every unique URL cited across the repo, cited-vs-surfaced-only status
  sources-ledger.json                # Same ledger, machine-readable
  sp800-53r4-to-r5-comparison-workbook.xlsx   # SP 800-53 Rev.4→Rev.5 control-mapping reference workbook
...
1. Clone the repo — no internet access is required afterward. All external guidance cited in `MAINTENANCE-PLAN.md` and the research file has a local copy under `reference/external-sources/`.
...
4. Use `reference/JSIG/control-families/INDEX.md` for the control inventory, and `reference/JSIG-source/chapter-3-*-family.md` for the full verbatim JSIG family text (real ODP values and SAP-specific guidance) -- the latter supersedes the former's NIST-boilerplate placeholders; see `reference/JSIG/appendices/EXTRACTION-LIMITATIONS.md` for what changed. `execution-plan/CONTROL-LANGUAGE-CROSSWALK.md` maps every Control ID in the Master Calendar to its real title.
...
## Known gaps (see also `reference/JSIG/appendices/EXTRACTION-LIMITATIONS.md` and `reference/JSIG-source/EXTRACTION-LOG.md`)

- Full verbatim JSIG family text and the complete 963-row Appendix C SAP baseline table have since been extracted (real ODP values and SAP-specific guidance, in `reference/JSIG-source/`) -- the earlier partial-extraction limitation is resolved; see `reference/JSIG/appendices/EXTRACTION-LIMITATIONS.md`.
